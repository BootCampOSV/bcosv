...
HEADER="""
<head>
    <title>OSV Parts</title>
    <meta name='Keywords' content='OSV,pythonOCC'>
    <meta charset="utf-8">
    <link rel="stylesheet" type="text/css" href="http://webgl.pythonocc.org/css/x3dom.css" charset="utf-8" ></link>
    <script type="text/javascript" src="http://webgl.pythonocc.org/js/x3dom-full.js"></script>
...
#    <meta charset="utf-8">
#    <link rel="stylesheet" type="text/css" href="@jspath@/css/x3dom.css" charset="utf-8" ></link>
#    <script type="text/javascript" src="@jspath@/js/x3dom-full.js"></script>
#    <script type="text/javascript" src="http://code.jquery.com/jquery-2.1.0.min.js" ></script>
#    <style type="text/css">
#        body {
...
def forge_body(filename,level,**kwargs):
    defaults ={ 'previous_name':''}
    WEB ="https://github.com/BootCampOSV/bcosv/tree/master/Doc/mechanics/stp/TABBY_EVO/2_seats/"
    for k in defaults:
        if k not in kwargs:
            kwargs[k] = defaults[k]
    ...
    <b>Open Source Vehicle Documentation</b><hr>
    <ul>
     """
    scurrent_name = "<li>Current Name: "+ filename+"</li>\n"
    sprevious_name = "<li>Previous Name: "+ kwargs['previous_name']+ "</li>\n"
    filestep = filename+'.stp'
    current_hash = hashlib.md5(open(filestep,'rb').read()).hexdigest()
    scurrent_hash = "<li>hash:"+current_hash +"</li>\n"
    sstepfile="<li>Step File: <a href="+WEB+"level"+str(level)+'/'+filestep+">"+filestep+"</a>"
    sref="<li>OEM Ref:</li>\n"
    smaterial="<li>Material:</li>\n"
    sweight="<li>Weight:</li>\n"
    #<li>Provider:</li>
    #<li>Price:</li>
    sparent="<li>Parent:</li>\n"
    schild="<li>Children:</li>\n"
    sneighbours="<li>Neighbors:</li>\n"
    #<li>Photo:</li>
    B2= """
        </ul>
        <a style="font-size:14px;" href=http://www.pythonocc.org> Powered by :  http://www.pythonocc.org</a>
    </div>
    ...
    """
    BODY = B1 + scurrent_name + sprevious_name + scurrent_hash + sstepfile +\
            sref + smaterial + sweight + sparent + schild + sneighbours + B2
    return BODY
...
class HTMLHeader(object):
    ...
    def get_str(self,local=True):
        if local:
            x3dom_build_location = os.sep.join([OCC.__path__[0], 'Display', 'WebGl'])
        else:
            x3dom_build_location = "http://webgl.pythonocc.org"
...
class HTMLBody(object):
    def __init__(self, background_color, x3d_filename,BODY):
        self._background_color = background_color
    ...
    def get_str(self):
        ...
        body_str = self.BODY.replace('@background-color@', '%s' % self._background_color)
...
class X3DomRenderer(object):
    def __init__(self, background_color="#123345", path=None,filename=None,level=1):
        self._background_color = background_color
        if not path:  # by default, write to a temp directory
            ...
            self._path = path
        if not filename:
            self._html_filename = os.path.join(self._path, 'webgl.html')
            self._x3d_filename = os.path.join(self._path, 'shape.x3d')
        else:
            self._html_filename = os.path.join(self._path, filename+'.html')
            self._x3d_filename = os.path.join(self._path, filename+'.x3d')
        self.BODY = forge_body(filename,level)
    ...
    def GenerateHTMLFile(self,local=True):
        ...
        # header
        fp.write(HTMLHeader(self._background_color).get_str(local))
        # body
        fp.write(HTMLBody(self._background_color, self._x3d_filename,self.BODY).get_str())
        fp.write("</html>\n")
        fp.close()
